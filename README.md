...
- **aerospace** — configuração do [AeroSpace](https://nikitabobko.github.io/AeroSpace/) (`aerospace.toml`), window manager em tiles. Modificador é a Hyper key (Caps Lock segurado — ver [karabiner/README.md](karabiner/README.md)): `Hyper+H/J/K/L` foco, `Hyper+7/8/9/0/-/=` troca de workspace por contexto (`DEV`/`WEB`/`COMM`/`DOCS`/`PERSONAL`/`MONITORING`), `Hyper+Tab` volta ao workspace anterior, `Hyper+A` alterna tiles/accordion. Todo o esquema de binds do AeroSpace depende do Karabiner sintetizando a Hyper key — trocar pro profile Nativo do Karabiner (kill switch) desliga o AeroSpace inteiro junto, não só os home row mods. `Hyper+W` entra no modo `window` — dentro dele, sem precisar da Hyper de novo: `H/J/K/L` move a janela, `Shift+H/J/K/L` troca de lugar com a vizinha (`swap`), `1..6` manda a janela pro workspace, `Esc` sai do modo. `Hyper+R` entra no modo `resize` (`H`/`L` ajusta largura, `J`/`K` ajusta altura, repete até `Esc`/`Enter`); `Hyper+Space` entra no modo `service` (`R` reseta o layout, `F` alterna floating/tiling da janela focada, `Q` fecha as outras janelas do workspace, `H`/`J`/`K`/`L` agrupa com a vizinha via `join-with`, `Esc`/`Enter` sai do modo — cada ação do `service` já volta sozinha pro modo `main`). Nos modos `window`, `resize` e `service`, toque (não segure) `j`/`k`/`l` — e no `service` também `f` — segurar por mais de 200ms aciona os home row mods do Karabiner em vez da ação do AeroSpace. Regras automáticas por app (`on-window-detected`) mandam WezTerm/Sublime Text/GitKraken/Claude pra `DEV`, Chrome/YouTube pra `WEB`, Calendário pra `COMM`, Obsidian/Notion/Preview pra `DOCS`; Ajustes do Sistema e F-Secure abrem flutuando. As regras valem só pra janelas novas — janelas já abertas quando a config recarrega ficam onde estão (mova com `Hyper+W` + número). `PERSONAL` e `MONITORING` ficam vazios por enquanto. `start-at-login = false` — inicie manualmente (`aerospace` ou `open -a AeroSpace`) até validar. **Atenção**: se existir um `~/.aerospace.toml` legado, remova-o antes (o AeroSpace lê ambos `~/.aerospace.toml` e `~/.config/aerospace/aerospace.toml` e rejeita config ambígua)
- **sketchybar** — barra de status ([SketchyBar](https://felixkratz.github.io/SketchyBar/), instalado via `brew tap FelixKratz/formulae`), `sketchybarrc` + `colors.sh` (paleta `coolnight`, mesma do Zellij) + `plugins/`. Mostra dois indicadores ligados ao AeroSpace: workspace focado (`DEV`/`WEB`/`COMM`/`DOCS`/`PERSONAL`/`MONITORING`, clicável pra trocar) e modo ativo (`MAIN`/`WINDOW`/`RESIZE`/`SERVICE`, cor muda por modo — resolve não saber se ficou "preso" num modo do AeroSpace). Atualiza via os callbacks `exec-on-workspace-change`/`on-mode-changed` do `aerospace.toml`. **Sem autostart ainda** — inicie manualmente (`sketchybar &`) até validar. **Passo manual pendente**: pra barra substituir de verdade a menu bar nativa, ative "Ocultar e mostrar automaticamente a barra de menus" (definida como "Sempre") em Ajustes do Sistema → Central de Controle — não é feito automaticamente por nenhum script deste pacote
