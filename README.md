...
- **aerospace** — configuração do [AeroSpace](https://nikitabobko.github.io/AeroSpace/) (`aerospace.toml`), window manager em tiles. Workspaces nomeados por contexto nas teclas `ctrl-alt-1..6` (`DEV`/`WEB`/`COMM`/`DOCS`/`PERSONAL`/`MONITORING`): layout `tiles`, gaps, foco/mover janela com `ctrl-alt-h/j/k/l` (+ `shift` pra mover), troca de workspace com `ctrl-alt-1..6` (+ `shift` pra mover a janela junto), `ctrl-alt-tab` volta ao workspace anterior. Regras automáticas por app (`on-window-detected`) mandam WezTerm/Sublime Text/GitKraken/Claude pra `DEV`, Chrome/YouTube pra `WEB`, Calendário pra `COMM`, Obsidian/Notion/Preview pra `DOCS`; Ajustes do Sistema e F-Secure abrem flutuando. As regras valem só pra janelas novas — janelas já abertas quando a config recarrega ficam onde estão (mova com `ctrl-alt-shift-N`). `PERSONAL` e `MONITORING` ficam vazios por enquanto. `start-at-login = false` — inicie manualmente (`aerospace` ou `open -a AeroSpace`) até validar. **Atenção**: se existir um `~/.aerospace.toml` legado, remova-o antes (o AeroSpace lê ambos `~/.aerospace.toml` e `~/.config/aerospace/aerospace.toml` e rejeita config ambígua). Ainda sem Hyper key
