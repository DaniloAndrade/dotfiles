...
- **aerospace** — configuração do [AeroSpace](https://nikitabobko.github.io/AeroSpace/) (`aerospace.toml`), window manager em tiles. Só a fase de fundamentos por enquanto: layout `tiles`, gaps, foco/mover janela com `ctrl-alt-h/j/k/l` (+ `shift` pra mover), troca de workspace com `ctrl-alt-1..6` (+ `shift` pra mover a janela junto), `ctrl-alt-tab` volta ao workspace anterior. `start-at-login = false` — inicie manualmente (`aerospace` ou `open -a AeroSpace`) até validar. **Atenção**: se existir um `~/.aerospace.toml` legado, remova-o antes (o AeroSpace lê ambos `~/.aerospace.toml` e `~/.config/aerospace/aerospace.toml` e rejeita config ambígua). Sem regras por app nem Hyper key ainda
